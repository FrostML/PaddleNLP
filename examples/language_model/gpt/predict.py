...
class Demo:
    ...
    def predict(self, batch_size):
        ...
        bos_id = self.tokenizer.convert_tokens_to_ids("<|endoftext|>")

        input_ids = np.array(
            [[bos_id] for i in range(batch_size * 1)]).astype("int64").reshape(
                [batch_size, 1])
        ...
        paddle.fluid.core._cuda_synchronize(place)
        print("Avg: ", (time.time() - start) / 50 * 1000)
...
if __name__ == "__main__":
    ...
    # else:
    demo = Demo("gpt", sys.argv[1])
    # demo.ask_question_en("Who is the CEO of Apple?")
    demo.predict(eval(sys.argv[2]))
